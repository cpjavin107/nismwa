...
public class ManagementZblockFragment extends BaseFragment implements View.OnClickListener {
    private RequestQueue requestQueue;
    private int page = 4;
    private String ownerId;
    ImageView imgBack, imgOne, imgTwo, imgThree, imgFour, imgFive, imgSix;
    AppCompatTextView txtOne, txtTwo, txtThree, txtFour, txtNext, txtTitle, txtFive, txtSix, txtPrevious;
    RelativeLayout rlFour, rlOne, rlTwo, rlThree, rlFive, rlSix;
    int fram;
                        ...
                        try {
                            ...
                            if (status == 1) {
                                if (candidateQuickJobsResponse.getDetailsArrayList().size() > 0) {
                                    ...
                                    if (size > 5) {
                                        final String id = candidateQuickJobsResponse.getDetailsArrayList().get(5).getMemberId().trim();
                                        String name = candidateQuickJobsResponse.getDetailsArrayList().get(5).getMc_name().trim();
                                        String desc = candidateQuickJobsResponse.getDetailsArrayList().get(5).getDesignationName().trim();
                                        String firm = candidateQuickJobsResponse.getDetailsArrayList().get(5).getFirm_name().trim();
                                        String address = candidateQuickJobsResponse.getDetailsArrayList().get(5).getAddress().trim();
                                        String phone = candidateQuickJobsResponse.getDetailsArrayList().get(5).getMobileNumber().trim();
                                        if (!TextUtils.isEmpty(candidateQuickJobsResponse.getDetailsArrayList().get(5).getImage().trim())) {
                                            String pic = candidateQuickJobsResponse.getDetailsArrayList().get(5).getImage().trim();
                                            Picasso.with(activity).load(pic).error(R.drawable.profile_icon).placeholder(R.drawable.profile_icon).transform(new CircleTransform()).into(imgSix);
                                        } else {
                                            Picasso.with(activity).load(R.drawable.profile_icon).transform(new CircleTransform()).into(imgSix);
                                        }

                                        if (!TextUtils.isEmpty(candidateQuickJobsResponse.getDetailsArrayList().get(5).getMc_name().trim())) {
                                            txtSix.setText(Utility.fromHtml("<font ><b>" + name + "</b><br>(" + desc + ")</font"));
                                        }
                                    ...
                                    if (size > 4) {
                                        final String id = candidateQuickJobsResponse.getDetailsArrayList().get(4).getMemberId().trim();
                                        String name = candidateQuickJobsResponse.getDetailsArrayList().get(4).getMc_name().trim();
                                        String desc = candidateQuickJobsResponse.getDetailsArrayList().get(4).getDesignationName().trim();
                                        String firm = candidateQuickJobsResponse.getDetailsArrayList().get(4).getFirm_name().trim();
                                        String address = candidateQuickJobsResponse.getDetailsArrayList().get(4).getAddress().trim();
                                        String phone = candidateQuickJobsResponse.getDetailsArrayList().get(4).getMobileNumber().trim();
                                        if (!TextUtils.isEmpty(candidateQuickJobsResponse.getDetailsArrayList().get(4).getImage().trim())) {
                                            String pic = candidateQuickJobsResponse.getDetailsArrayList().get(4).getImage().trim();
                                            Picasso.with(activity).load(pic).error(R.drawable.profile_icon).placeholder(R.drawable.profile_icon).transform(new CircleTransform()).into(imgFive);
                                        } else {
                                            Picasso.with(activity).load(R.drawable.profile_icon).transform(new CircleTransform()).into(imgFive);
                                        }

                                        if (!TextUtils.isEmpty(candidateQuickJobsResponse.getDetailsArrayList().get(4).getMc_name().trim())) {
                                            txtFive.setText(Utility.fromHtml("<font ><b>" + name + "</b><br>(" + desc + ")</font"));
                                        }
                                    ...
                                    if (size > 3) {
                                        final String id = candidateQuickJobsResponse.getDetailsArrayList().get(3).getMemberId().trim();
                                        String name = candidateQuickJobsResponse.getDetailsArrayList().get(3).getMc_name().trim();
                                        String desc = candidateQuickJobsResponse.getDetailsArrayList().get(3).getDesignationName().trim();
                                        String firm = candidateQuickJobsResponse.getDetailsArrayList().get(3).getFirm_name().trim();
                                        String address = candidateQuickJobsResponse.getDetailsArrayList().get(3).getAddress().trim();
                                        String phone = candidateQuickJobsResponse.getDetailsArrayList().get(3).getMobileNumber().trim();
                                        if (!TextUtils.isEmpty(candidateQuickJobsResponse.getDetailsArrayList().get(3).getImage().trim())) {
                                            String pic = candidateQuickJobsResponse.getDetailsArrayList().get(3).getImage().trim();
                                            Picasso.with(activity).load(pic).error(R.drawable.profile_icon).placeholder(R.drawable.profile_icon).transform(new CircleTransform()).into(imgFour);
                                        } else {
                                            Picasso.with(activity).load(R.drawable.profile_icon).transform(new CircleTransform()).into(imgFour);
                                        }

                                        if (!TextUtils.isEmpty(candidateQuickJobsResponse.getDetailsArrayList().get(3).getMc_name().trim())) {
                                            txtFour.setText(Utility.fromHtml("<font ><b>" + name + "</b><br>(" + desc + ")</font"));
                                        }
    ...
    private void initialize(View view) {
        ...
        txtTitle.setText("Executive Committee(Z-Block)");
        ...
        txtFour.setTypeface(FontRobotoCondensedRegularSingleTonClass.getInstance(activity).getTypeFace());
        txtFive = view.findViewById(R.id.txtFive);
        txtFive.setTypeface(FontRobotoCondensedRegularSingleTonClass.getInstance(activity).getTypeFace());
        txtSix = view.findViewById(R.id.txtSix);
        txtSix.setTypeface(FontRobotoCondensedRegularSingleTonClass.getInstance(activity).getTypeFace());
        ...
        txtPrevious.setTypeface(FontRobotoCondensedRegularSingleTonClass.getInstance(activity).getTypeFace());

        rlOne = view.findViewById(R.id.rlOne);
        rlTwo = view.findViewById(R.id.rlTwo);
        rlThree = view.findViewById(R.id.rlThree);
        rlFour = view.findViewById(R.id.rlFour);
        rlFive = view.findViewById(R.id.rlFive);
        rlSix = view.findViewById(R.id.rlSix);
    }

    @Override
    protected int getFragmentLayout() {
        return R.layout.management_two_layout;
    }
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.imgBack:
                ...
                break;
            case R.id.txtNext:
                ManagementNominatedFragment managementNominatedFragment = new ManagementNominatedFragment();
                callFragmentMethod(managementNominatedFragment, Constants.FRG_MANAGEMENT_NOMINATED_LIST, fram);
                break;
