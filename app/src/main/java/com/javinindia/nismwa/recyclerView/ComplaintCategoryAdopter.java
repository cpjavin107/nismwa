...
public class ComplaintCategoryAdopter extends RecyclerView.Adapter<ComplaintCategoryAdopter.ViewHolder> {

    List<TypeDetail> list;
    Context context;
    MyClickListener myClickListener;
    ArrayList<TypeDetail> shopCategoryListArrayList;

    public ComplaintCategoryAdopter(Context context) {
    ...
    public void onBindViewHolder(final ViewHolder viewHolder, final int position) {
        final TypeDetail categoryDetail = (TypeDetail) list.get(position);
        if (!TextUtils.isEmpty(categoryDetail.getType_name().trim())) {
            String cityName = categoryDetail.getType_name().trim();
    ...
    public interface MyClickListener {
        void onItemCatClick(int position, TypeDetail model);
